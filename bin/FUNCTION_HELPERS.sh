...
displayTestErrorTexts() {
  ...
  echo -e "Remember that your brand new repository should be created from here: ${BOLD}${UPURPLE}https://github.com/quangsuong/project-template/generate${NC}"
...
displayHelpTexts() { # (it will manually detect your git data and prompt for the project type)
  ...
  echo -e "  bash $0 -u=quangsuong --projectType=Github-template --omit-commit${NC}\n"

  echo -e "The username, project-name and email are automatically gathered from your git repository and git config."
  echo -e "Make sure you have ${BBLUE}read the documentation before executing${NC} this script: ${UPURPLE}https://github.com/quangsuong/project-template${NC}"
  echo -e "If you have any questions or if any issue is found, please make sure to report it at: ${UPURPLE}https://github.com/quangsuong/project-template/issues${NC}"
}
...
$PROJECT_NAME was generated from *[quangsuong/project-template](https://github.com/quangsuong/project-template)* 📚
...
* The basic project structure from **[quangsuong/project-template](https://github.com/quangsuong/project-template)**.
EOF"
}

...
# PURPOSE:       Shell script that setups the @quangsuong/project-template GitHub project template.
...
echo -e "Thanks for using ${GREEN}@quangsuong/project-template${NC}"
echo -e "Read all the documentation carefully before you continue executing this script: ${UPURPLE}https://github.com/quangsuong/project-template${NC}\n"
...
y | Y)
  ...
  find .github/ -type f -name "*" -print0 | xargs -0 sed -i "s/quangsuong/${NEW_USERNAME}/g"
  ...
  find .gitignore -type f -name "*" -print0 | xargs -0 sed -i "s/quangsuong\/project-template/${NEW_USERNAME}\/${PROJECT_NAME}/g"
  ...
  if [ $will_omit_commit = false ]; then                                                  # if the ignore option for tests has been specified
    ...
    git commit -m "📝 Set up '@quangsuong/project-template' template: Personalized files by executing the SETUP_TEMPLATE.sh script.🚀"
